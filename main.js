        ...
        if (
            ...
            if (collisionWithObstacle) {  //якщо зіткнення відбулось
                ...
                if (hearts <= 0) {
                    isGameOver = true;
                    updateMainScore();
                    localStorage.setItem('scoreUser', score)  //додала ще
...
function startGame() {
    localStorage.clear();  //при перевантаженні сторінки в локальній пам'яті зберігаються бали, а при старті очищується
    score = 0;
